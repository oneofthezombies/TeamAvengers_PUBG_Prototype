...
PlayerParts::PlayerParts(float rotXSpeed)
{
...
void PlayerParts::Update()
{
    ...
    else if(m_ePartTag == tGun)    ////////추가된 부분
    {
        m_rotXAngle = -D3DX_PI / 2.0f;
    }

    if (m_ePartTag == tEmpty)
    {
...
void PlayerParts::SetPCVertex(vector<VERTEX_PC>& vecOut, vector<D3DXVECTOR3>& vecVertex)
{
...
void PlayerParts::SetPTVertex(vector<VERTEX_PT>& vecOut, vector<D3DXVECTOR3>& vecVertex, vector<vector<int>>& uv)
{
...
void PlayerParts::SetMovingState(bool isMoving)
{
    ...
    for (auto child : m_vecPChild)
    {
        static_cast<PlayerParts*>(child)
            ->SetMovingState(m_isMoving);
    }
}

void PlayerParts::MakeUVList(vector<D3DXVECTOR2>& out,
...
void PlayerParts::SetPartTag(PartTag a)
{
    m_ePartTag = a;
}

PartTag PlayerParts::GetPartTag()
{
    return m_ePartTag;
}

void PlayerParts::SetRotXspeed(float a)
{
    m_rotXSpeed = a;
}

float PlayerParts::GetRotXspeed()
{
    return m_rotXSpeed;
}

void PlayerParts::SyncMotion()
{
    float temp = static_cast<PlayerParts*>(m_pParent->GetChildVec()[1])->m_rotXAngle;
